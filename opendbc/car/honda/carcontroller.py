...
class CarController(CarControllerBase, MadsCarController, GasInterceptorCarController, IntelligentCruiseButtonManagementInterface):
  ...
  def update(self, CC, CC_SP, CS, now_nanos):
    ...
    if CC.longActive:
      accel = CarControllerExt.accel(self, CS, actuators.accel)
      gas, brake = CarControllerExt.compute_gas_brake(self, CS, actuators.accel)
    else:
      accel = 0.0
      gas, brake = 0.0, 0.0

    # *** rate limit steer ***
    limited_torque = rate_limit(CarControllerExt.convert_torque_to_torque_cmd(self, CC, CS, now_nanos, actuators.torque), self.last_torque, -self.params.STEER_DELTA_DOWN * DT_CTRL,
    ...
    self.brake_last = CarControllerExt.rate_limit_3_up(self, pre_limit_brake, self.brake_last, -2., DT_CTRL)
    ...
    CarControllerExt.update_speed_control(self, CS, accel)
    ...
    CarControllerExt.update_wind_brake(self, CS)
    # all of this is only relevant for HONDA NIDEC
    CarControllerExt.zero_speed_control(self)
    max_accel = interp(CS.out.vEgo, self.params.NIDEC_MAX_ACCEL_BP, self.params.NIDEC_MAX_ACCEL_V) # RoenPilot re-code
    ...
    if not self.CP.openpilotLongitudinalControl:
      ...
      if self.frame % 2 == 0:
        ...
        if self.CP.carFingerprint in HONDA_BOSCH:
          CarControllerExt.update_self_accel(self, CS, accel)
          CarControllerExt.update_self_gas(self, actuators.LongControlState, CS, accel)

          stopping = actuators.longControlState == LongCtrlState.stopping
          self.stopping_counter = self.stopping_counter + 1 if stopping else 0
          can_sends.extend(hondacan.create_acc_commands(self.packer, self.CAN, CC.enabled, CC.longActive, self.accel, self.gas,
                                                        self.stopping_counter, self.CP.carFingerprint, self.gas_pedal_force))
        else:
          apply_brake = CarControllerExt.apply_brake(self)
          ...
          can_sends.extend(CarControllerExt.GasInterceptorCarController_update(self, CC, CS, actuators, gas, brake))

    # Send dashboard UI commands.
    if self.frame % 10 == 0:
      if self.CP.openpilotLongitudinalControl:
        # On Nidec, this also controls longitudinal positive acceleration
        can_sends.append(CarControllerExt.create_acc_hud(self, CC, CS, pcm_speed, pcm_accel, hud_control, hud_v_cruise))

      steering_available = CS.out.cruiseState.available and CS.out.vEgo > max(self.params.STEER_GLOBAL_MIN_SPEED, self.CP.minSteerSpeed)
      can_sends.extend(CarControllerExt.create_lkas_hud(self, CC, CS, hud_control, alert_steer_required, apply_torque, steering_available))
    ...
    new_actuators.gas, new_actuators.brake = CarControllerExt.set_new_actuator_gas_brake(self)
